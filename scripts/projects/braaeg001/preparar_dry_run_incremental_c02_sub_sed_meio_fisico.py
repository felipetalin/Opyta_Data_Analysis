...
style_workbook_base = mf.style_workbook
...
def latest_staging() -> Path:
    files = sorted(OUTPUT_DIR.glob("*_staging_resultados_meio_fisico_c02_subterranea_sedimentos.xlsx"))
    if not files:
        raise FileNotFoundError(f"Nenhum staging encontrado em {OUTPUT_DIR}")
...
def fetch_existing() -> tuple[pd.DataFrame, pd.DataFrame]:
...
def build_insert_sql(preview: pd.DataFrame) -> list[str]:
    lines = [
        "-- BRAAEG001 C02 Agua Subterranea + Sedimento dry-run incremental.",
...
def build_rollback_sql() -> list[str]:
    matrix_values = ", ".join(sql_literal(matrix) for matrix in MATRICES)
    return [
        "-- Rollback planejado para remover somente a C02 Subterranea/Sedimento do BRAAEG001.",
...
def main() -> int:
    ...
    preview, mapping_issues = build_migration_preview(staging, lookup)

    existing_summary, existing_keys = fetch_existing()
    ...
    matrix_summary = (
        preview.groupby(["matriz", "nome_campanha"], as_index=False)
        .agg(registros=("nome_parametro", "size"), pontos=("nome_ponto", "nunique"), parametros=("nome_parametro", "nunique"))
        .sort_values(["matriz", "nome_campanha"])
    )
    summary = pd.DataFrame(
        [
            {"Metrica": "registros_planejados_c02", "Valor": len(preview)},
            {"Metrica": "matrizes_planejadas_c02", "Valor": preview["matriz"].nunique()},
            ...
            {"Metrica": "parametros_com_vmp", "Valor": int(preview[[col for col in VMP_COLS if col in preview.columns]].notna().any(axis=1).sum())},
            {"Metrica": "status", "Valor": "READY_FOR_APPLY_APPROVAL" if mapping_issues.empty and duplicate_planned.empty else "REVIEW_REQUIRED"},
        ]
    )

    dry_run_xlsx = OUTPUT_DIR / f"{ts}_dry_run_incremental_c02_subterranea_sedimentos_braaeg001.xlsx"
    apply_sql = OUTPUT_DIR / f"{ts}_apply_incremental_c02_subterranea_sedimentos_braaeg001.sql"
    rollback_sql = OUTPUT_DIR / f"{ts}_rollback_incremental_c02_subterranea_sedimentos_braaeg001.sql"
    log_path = LOG_DIR / f"{ts}_dry_run_incremental_c02_subterranea_sedimentos_braaeg001.json"

    write_excel(
        dry_run_xlsx,
        {
            "resumo": summary,
            "resumo_matriz": matrix_summary,
            ...
            "alias_usados": aliases,
        },
    )
    apply_sql.write_text("\n".join(build_insert_sql(preview)), encoding="utf-8")
    rollback_sql.write_text("\n".join(build_rollback_sql()), encoding="utf-8")

    manifest = {
        "timestamp": ts,
        "status": clean_text(summary.loc[summary["Metrica"].eq("status"), "Valor"].iloc[0]),
