...
from opyta_analysis.pipelines.diagnostico.zooplancton import _load_zooplancton_df  # noqa: E402


PROJECT_CODE = "WSPKIN001"
PROJECT_ID = 211
GROUP = "Zooplancton"
GROUP_DISPLAY = "Zooplâncton"
OUTPUT_DIR = Path("G:/Meu Drive/Opyta/Clientes/Clientes/Clientes/WSP/BAndeirinhas_Kinross/Resultados/Zooplancton")
OUTPUT_DIR = Path(os.environ.get("WSPKIN001_ZOO_OUTPUT_DIR", str(OUTPUT_DIR)))
AUDIT_DIR = ROOT / "outputs" / "_project_scripts" / "WSPKIN001__kinross_bandeirinhas" / "zooplancton"
...
GRID = "#D8E9D4"
PHYLA_COLORS = ["#11420C", "#19FF00", "#6A8F63", "#2A6F97", "#C77D32", "#6D597A", "#4D908E", "#9C6644"]

FINAL_FIGURES = [
    "02_grafico_riqueza_por_ponto_zooplancton.png",
    "03_grafico_densidade_total_por_ponto_zooplancton.png",
    "04_grafico_riqueza_filo_barras_zooplancton.png",
    "05_grafico_riqueza_filo_rosca_zooplancton.png",
    "06_grafico_densidade_filo_por_ponto_zooplancton.png",
    "07_grafico_densidade_relativa_filo_por_ponto_zooplancton.png",
    "08_grafico_densidade_por_taxon_zooplancton.png",
    "09_grafico_densidade_taxon_ponto_zooplancton.png",
    "10_grafico_diversidade_alfa_zooplancton.png",
    "11_dendrograma_similaridade_zooplancton.png",
    "11B_dendrograma_similaridade_pontos_zooplancton.png",
    "12_curva_suficiencia_amostral_zooplancton.png",
    "13_mini_mapa_taxons_bioindicadores_zooplancton.png",
    "14_grafico_sintese_zooplancton.png",
]
...
def should_italicize_taxon(label: str) -> bool:
...
def apply_taxon_tick_styles(ax) -> None:
    for tick in ax.get_yticklabels():
        tick.set_fontstyle("italic" if should_italicize_taxon(tick.get_text()) else "normal")
...
def ensure_nonempty(df: pd.DataFrame) -> None:
    if df.empty:
        raise RuntimeError("A fatia consolidada de Zooplâncton retornou vazia.")


def load_data(env_file: str | None = ".env") -> pd.DataFrame:
    df = _load_zooplancton_df(project_id=PROJECT_ID, group=GROUP, env_file=env_file)
    ensure_nonempty(df)
    df = df.copy()
...
def density_unit(df: pd.DataFrame) -> str:
    ...
    unit = first_valid(q.get("unidade_esforco", pd.Series(dtype=object)), default="org/amostra")
    return unit or "org/amostra"
...
def plot_taxon_horizontal(df_taxon: pd.DataFrame, *, out_name: str, unit: str) -> Path:
    top_taxa = df_taxon.groupby("nome_cientifico")["densidade"].sum().sort_values(ascending=False).head(20).index.tolist()
    plot_df = df_taxon[df_taxon["nome_cientifico"].isin(top_taxa)].copy()
    taxa_order = plot_df.groupby("nome_cientifico")["densidade"].sum().sort_values(ascending=True).index.tolist()
    campaigns = sorted(df_taxon["nome_campanha"].unique().tolist(), key=campaign_key)
    pivot = (
        plot_df.pivot_table(index="nome_cientifico", columns="nome_campanha", values="densidade", aggfunc="sum", fill_value=0)
    ...
    for i, (ax, campaign) in enumerate(zip(axes, campaigns)):
        ...
        for bar, value in zip(bars, values):
            if value <= 0:
                continue
            ax.text(float(value) + xmax * 0.014, bar.get_y() + bar.get_height() / 2, fmt_number(value, 2), va="center", ha="left", fontsize=12)
        ax.set_yticks(y)
        ax.set_yticklabels(labels)
        apply_taxon_tick_styles(ax)
        ...
        if ax is not axes[0]:
            ax.tick_params(labelleft=False)
    fig.subplots_adjust(left=0.36, right=0.985, bottom=0.14, top=0.84, wspace=0.12)
    return save_fig(fig, out_name)


def plot_taxon_heatmap(df: pd.DataFrame, *, campaigns: list[str], points: list[str], out_name: str, unit: str) -> tuple[Path, pd.DataFrame]:
    q = quantitative_df(df)
    ...
    for ax, campaign in zip(axes, campaigns):
        ...
        values = plot_mat.reindex(columns=local_cols, fill_value=0).to_numpy(dtype=float)
        ...
        ax.set_yticklabels(plot_mat.index.tolist(), fontsize=14)
        apply_taxon_tick_styles(ax)
        ...
        for i in range(values.shape[0]):
            for j in range(values.shape[1]):
                ...
                ax.text(j, i, fmt_number(value, 1), ha="center", va="center", fontsize=9, color=color)
        for spine in ["top", "right", "left", "bottom"]:
            ax.spines[spine].set_visible(True)
            ax.spines[spine].set_color("black")
            ax.spines[spine].set_linewidth(1.2)
    cbar_ax = fig.add_axes([0.855, 0.20, 0.018, 0.58])
    cbar = fig.colorbar(im, cax=cbar_ax)
    cbar.set_label(f"Densidade ({unit})", rotation=90, labelpad=8)
    fig.subplots_adjust(left=0.34, right=0.82, bottom=0.18, top=0.84, wspace=0.12)
...
def plot_synthesis(sheets: dict[str, pd.DataFrame], unit: str, out_name: str) -> Path:
    ...
    fig.suptitle("Síntese do zooplâncton - WSPKIN001", fontweight="bold", fontsize=15)
    return save_fig(fig, out_name)
...
def make_contact_sheet(tag: str) -> Path:
    ...
    out = AUDIT_DIR / f"{tag}_contact_sheet_figuras_zooplancton_a4_paisagem.png"
    sheet.save(out)
    return out


def write_html_report(metrics: dict, unit: str) -> Path:
    out = OUTPUT_DIR / "relatorio_tecnico_zooplancton_wspkin001.html"
    figures = [
        ("02_grafico_riqueza_por_ponto_zooplancton.png", "Riqueza por ponto"),
        ("03_grafico_densidade_total_por_ponto_zooplancton.png", "Densidade total por ponto"),
        ("04_grafico_riqueza_filo_barras_zooplancton.png", "Riqueza por filo"),
        ("06_grafico_densidade_filo_por_ponto_zooplancton.png", "Densidade por filo"),
        ("08_grafico_densidade_por_taxon_zooplancton.png", "Densidade por táxon"),
        ("09_grafico_densidade_taxon_ponto_zooplancton.png", "Densidade por táxon e amostra"),
        ("10_grafico_diversidade_alfa_zooplancton.png", "Diversidade alfa"),
        ("11_dendrograma_similaridade_zooplancton.png", "Similaridade"),
        ("11B_dendrograma_similaridade_pontos_zooplancton.png", "Similaridade por ponto"),
        ("12_curva_suficiencia_amostral_zooplancton.png", "Suficiência amostral"),
        ("14_grafico_sintese_zooplancton.png", "Síntese"),
    ]
    figure_blocks = []
...
<head>
  <meta charset="utf-8">
  <title>WSPKIN001 - Relatório técnico de zooplâncton</title>
  <style>
...
<body>
  <h1>WSPKIN001 - Diagnóstico do zooplâncton</h1>
  ...
  <p class="note">Riqueza, ocorrência e suficiência amostral usam registros qualitativos e quantitativos. Densidade, diversidade e similaridade usam somente registros quantitativos, pois a contagem consolidada de Zooplâncton já representa a densidade migrada.</p>

  <h2>Figuras principais</h2>
  {''.join(figure_blocks)}

  <h2>Rastreabilidade</h2>
  <p>Manifesto: <code>manifesto_entrega_zooplancton_wspkin001.json</code> e <code>manifesto_entrega_zooplancton_wspkin001.xlsx</code>.</p>
</body>
</html>
"""
...
def build_manifest_and_validation(generated_files: list[Path], metrics: dict, tag: str) -> tuple[Path, Path, Path, Path]:
    manifest_json = OUTPUT_DIR / "manifesto_entrega_zooplancton_wspkin001.json"
    manifest_xlsx = OUTPUT_DIR / "manifesto_entrega_zooplancton_wspkin001.xlsx"
    manifest_md = OUTPUT_DIR / "manifesto_entrega_zooplancton_wspkin001.md"
    validation_json = OUTPUT_DIR / "validacao_entrega_zooplancton_wspkin001.json"

    files = [p for p in sorted(OUTPUT_DIR.iterdir(), key=lambda x: x.name.lower()) if p.is_file() and p.name != "desktop.ini" and not p.name.startswith("manifesto_entrega_zooplancton_wspkin001")]
    rows = []
    for path in files:
        rows.append(
            {
    ...
    validation = {
        ...
        "warnings": [
            ...
            "Figuras 08 e 09 mostram os 20 táxons com maior densidade para preservar legibilidade em A4 paisagem; as planilhas mantêm todos os táxons.",
            "Figura 11B adicionada com similaridade de Bray-Curtis por ponto, sem distinguir campanha.",
            "Figura 13 adicionada como minimapa de ocorrência/riqueza de táxons associados à bioindicação; não representa diagnóstico isolado de qualidade ambiental.",
        ],
    }
    ...
    manifest = {
        ...
        "revision": {
            "id": "FITO_GERACAO_01",
            "applied_at": now_iso(),
            "description": "Resultados de Zooplâncton gerados em A4 paisagem com padrão visual de ictiofauna e premissas técnicas do grupo.",
            "audit_tag": tag,
        },
    }
    ...
    lines = [
        "# WSPKIN001 - Manifesto de entrega - Zooplâncton",
        "",
...
def write_synthesis_workbook(sheets: dict[str, pd.DataFrame]) -> Path:
    out = OUTPUT_DIR / "14_tabela_sintese_zooplancton.xlsx"
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
...
def generate(env_file: str | None = ".env") -> dict:
    ...
    generated.append(write_excel(df.drop(columns=["tipo_norm", "campanha_label", "amostra_label"], errors="ignore"), "00_base_consolidada_zooplancton.xlsx", sheet_name="base_consolidada"))

    composition = build_composition(df)
    generated.append(write_excel(composition, "01_tabela_composicao_zooplancton.xlsx", sheet_name="composicao"))

    occurrence = build_occurrence(df, campaigns, points)
    generated.append(write_excel(occurrence, "01_tabela_ocorrencia_zooplancton.xlsx", sheet_name="ocorrencia"))

    richness_point = build_richness_by_point(df, campaigns, points)
    generated.append(write_excel(richness_point, "02_df_riqueza_por_ponto_zooplancton.xlsx"))
    generated.append(plot_grouped_campaign_bars(richness_point, value_col="riqueza", ylabel="Riqueza taxonômica", out_name="02_grafico_riqueza_por_ponto_zooplancton.png", decimals=0, integer_axis=True))

    density_point = build_density_by_point(df, campaigns, points)
    generated.append(write_excel(density_point, "03_df_densidade_total_por_ponto_zooplancton.xlsx"))
    generated.append(plot_grouped_campaign_bars(density_point, value_col="densidade_total", ylabel=f"Densidade total ({unit})", out_name="03_grafico_densidade_total_por_ponto_zooplancton.png", decimals=1))

    richness_phylum = build_richness_by_phylum(df)
    generated.append(write_excel(richness_phylum, "04_df_riqueza_por_filo_zooplancton.xlsx"))
    generated.append(plot_taxon_bar(richness_phylum, category_col="Filo", value_col="Número de táxons", xlabel="Filo", ylabel="Número de táxons", out_name="04_grafico_riqueza_filo_barras_zooplancton.png"))
    generated.append(plot_donut(richness_phylum, category_col="Filo", value_col="Número de táxons", out_name="05_grafico_riqueza_filo_rosca_zooplancton.png"))

    density_phylum, density_phylum_rel = build_density_phylum(df, campaigns, points)
    generated.append(write_excel(density_phylum, "06_df_densidade_filo_por_ponto_zooplancton.xlsx"))
    generated.append(plot_stacked_phylum(density_phylum, campaigns=campaigns, points=points, value_label=f"Densidade ({unit})", out_name="06_grafico_densidade_filo_por_ponto_zooplancton.png"))
    generated.append(write_excel(density_phylum_rel, "07_df_densidade_relativa_filo_por_ponto_zooplancton.xlsx"))
    generated.append(plot_stacked_phylum(density_phylum_rel, campaigns=campaigns, points=points, value_label="Densidade relativa (%)", out_name="07_grafico_densidade_relativa_filo_por_ponto_zooplancton.png", relative=True))

    density_taxon = build_density_taxon(df, campaigns)
    generated.append(write_excel(density_taxon, "08_df_densidade_por_taxon_zooplancton.xlsx"))
    generated.append(plot_taxon_horizontal(density_taxon, out_name="08_grafico_densidade_por_taxon_zooplancton.png", unit=unit))

    heatmap_png, heatmap_df = plot_taxon_heatmap(df, campaigns=campaigns, points=points, out_name="09_grafico_densidade_taxon_ponto_zooplancton.png", unit=unit)
    generated.append(write_excel(heatmap_df, "09_df_densidade_taxon_ponto_zooplancton.xlsx"))
    generated.append(heatmap_png)

    diversity_all, diversity_general = build_diversity(df, campaigns, points)
    generated.append(write_excel(diversity_all, "10_df_diversidade_alfa_zooplancton.xlsx"))
    generated.append(plot_diversity(diversity_all, campaigns=campaigns, points=points, out_name="10_grafico_diversidade_alfa_zooplancton.png"))

    mat_quant = build_quant_matrix(df, campaigns=campaigns, points=points, positive_only=True)
    mat_quant.index.name = "Amostra"
    generated.append(write_excel(mat_quant.reset_index(), "11_df_matriz_comunidade_zooplancton.xlsx"))
    dendro_png, dist_sq = plot_dendrogram(mat_quant, "11_dendrograma_similaridade_zooplancton.png")
    if dist_sq is not None:
        dist_sq.index.name = "Amostra"
        generated.append(write_excel(dist_sq.reset_index(), "11_df_distancias_braycurtis_zooplancton.xlsx"))
    ...
    generated.append(write_excel(mat_points.reset_index(), "11B_df_matriz_comunidade_pontos_zooplancton.xlsx"))
    dendro_points_png, dist_points_sq = plot_dendrogram(mat_points, "11B_dendrograma_similaridade_pontos_zooplancton.png")
    if dist_points_sq is not None:
        dist_points_sq.index.name = "Ponto"
        generated.append(write_excel(dist_points_sq.reset_index(), "11B_df_distancias_braycurtis_pontos_zooplancton.xlsx"))
    ...
    generated.append(write_excel(suff_curve, "12_df_curva_suficiencia_zooplancton.xlsx"))
    suff_png = plot_sufficiency(suff_curve, "12_curva_suficiencia_amostral_zooplancton.png")
    ...
    export_darwincore_ief(
        ...
        output_filename="DarwinCore_IEF_Zooplancton_WSPKIN001.xlsx",
    ...
    generated.append(plot_synthesis(synthesis_sheets, unit, "14_grafico_sintese_zooplancton.png"))
    ...
    audit = {
        ...
        "project_id": PROJECT_ID,
        "group": GROUP_DISPLAY,
        "action": "generate_zooplancton_results_a4_landscape",
        "output_dir": str(OUTPUT_DIR),
    ...
    audit_json = AUDIT_DIR / f"{tag}_geracao_resultados_zooplancton_a4_paisagem.json"
    audit_xlsx = AUDIT_DIR / f"{tag}_geracao_resultados_zooplancton_a4_paisagem.xlsx"
    ...
    metadata = {
        ...
        "project_id": PROJECT_ID,
        "group": GROUP_DISPLAY,
        "pipeline": "project_specific_zooplancton_a4_landscape",
