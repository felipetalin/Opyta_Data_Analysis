...
OUTPUT_DIR = CLIENT_ROOT / "Resultados" / "Zooplancton"
OUTPUT_DIR = Path(os.environ.get("WSPKIN001_ZOO_OUTPUT_DIR", str(OUTPUT_DIR)))
AUDIT_DIR = Path("outputs/_project_scripts/WSPKIN001__kinross_bandeirinhas/zooplancton")
...
ASSOCIATIONS = {
    "Ampla tolerância ecológica": {
        "color": PRIMARY,
        "taxa_label": r"$\it{Arcella}$ spp.; $\it{Difflugia}$ spp.; Bdelloidea",
    },
    "Matéria orgânica": {
        "color": SECONDARY,
        ...
        text(
            ...
            where id_projeto = :project_id
              ...
              and (
                ...
                or nome_cientifico in ('Bdelloida', 'Bdelloidea')
...
def classify_records(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        groups: list[str] = []
        if genus in {"Arcella", "Difflugia"} or taxon in {"Bdelloida", "Bdelloidea"}:
    ...
    for row_idx, campaign in enumerate(campaigns):
        for col_idx, association in enumerate(associations):
            ...
            for polygon in ael_polygons:
                ...
                ax.fill(coords[:, 0], coords[:, 1], facecolor="#F3F7EF", edgecolor=PRIMARY, linewidth=0.65, alpha=0.55, zorder=0)
