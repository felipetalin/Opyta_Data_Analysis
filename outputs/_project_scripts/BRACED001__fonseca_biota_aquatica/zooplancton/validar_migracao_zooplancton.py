...
TAXON_MAP = {"Ciliado NI": "Ciliado ni"}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate BRACED001 zooplankton migration and consolidation.")
    parser.add_argument("--source", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--opyta-data-root", required=True, type=Path)
    args = parser.parse_args()

    source_results = pd.read_excel(args.source, sheet_name="Resultados_Zooplancton")
    ...
    source_efforts["Esforco"] = pd.to_numeric(
        source_efforts["Esforco"].astype(str).str.extract(r"([-+]?\d+(?:[.,]\d+)?)", expand=False).str.replace(",", ".", regex=False),
        errors="coerce",
    )
    ...
    with engine.connect() as connection:
        db_results = pd.read_sql(
            text(
                ...
                SELECT c.nome_campanha AS "Campanha", p.nome_ponto AS "Ponto",
                       ...
                       sp.nome_cientifico AS "Nome_Cientifico", rz.numero_de_individuos AS contagem
                ...
                WHERE p.id_projeto = 133 AND e.grupo_biologico = 'Zooplâncton'
                """
            ),
            connection,
        )
        ...
        consolidated = pd.read_sql(
            text(
                ...
                WHERE id_projeto = 133 AND grupo_biologico = 'Zooplâncton'
                """
            ),
            connection,
        )
    ...
    result_diff = compare_values(source_results, db_results, keys, "Numero_de_Individuos", "contagem", "fonte_vs_base")
    consolidated_diff = compare_values(source_results, consolidated, keys, "Numero_de_Individuos", "contagem", "fonte_vs_consolidado")

    effort_keys = ["Campanha", "Ponto", "Metodo_de_Captura"]
    source_effort_agg = source_efforts.groupby(effort_keys, as_index=False).agg(
        Esforco=("Esforco", "first"),
        Unidade_Esforco=("Unidade_Esforco", "first"),
        Tipo_de_Amostragem=("Tipo_de_Amostragem", lambda values: "Mista" if len(set(values)) > 1 else next(iter(values))),
    )
    effort_diff = source_effort_agg[effort_keys + ["Tipo_de_Amostragem", "Esforco", "Unidade_Esforco"]].merge(
        db_efforts[effort_keys + ["Tipo_de_Amostragem", "esforco", "Unidade_Esforco"]],
        on=effort_keys + ["Tipo_de_Amostragem"], how="outer", indicator=True
    )
    ...
    summary = {
        ...
        "fonte_esforcos": len(source_effort_agg),
    ...
    base = args.output_dir / f"{stamp}_auditoria_pos_migracao_zooplancton_braced001"
