...
def main() -> int:
    parser = argparse.ArgumentParser(description="Valida a migracao e consolidacao de Zoobentos do BRACED001.")
    parser.add_argument("--source", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--opyta-data-root", required=True, type=Path)
    args = parser.parse_args()

    source_results = pd.read_excel(args.source, sheet_name="Resultados_Zoobentos")
    ...
    source_results["Nome_Cientifico"] = clean_series(source_results["Nome_Cientifico"])
    ...
    source_results["Numero_de_Individuos"] = pd.to_numeric(
        source_results["Numero_de_Individuos"], errors="coerce"
    ).fillna(0)
    ...
    with engine.connect() as connection:
        db_results = pd.read_sql(
            text(
                ...
                SELECT c.nome_campanha AS "Campanha", p.nome_ponto AS "Ponto",
                       ...
                       COALESCE(rz.tipo_amostragem, e.tipo_amostragem, e.tipo_de_amostragem) AS "Tipo_de_Amostragem",
                       sp.nome_cientifico AS "Nome_Cientifico", rz.abundancia
            ...
            connection,
            params={"project_id": PROJECT_ID, "group_name": GROUP},
        )
        ...
        db_points = pd.read_sql(
            text(
                """
                SELECT c.nome_campanha AS "Campanha", p.nome_ponto AS "Ponto",
                       p.latitude, p.longitude, p.data_hora_coleta
        ...
        consolidated = pd.read_sql(
            text(
                """
                SELECT nome_campanha AS "Campanha", nome_ponto AS "Ponto",
                       metodo_de_captura AS "Metodo_de_Captura",
                       tipo_amostragem AS "Tipo_de_Amostragem",
                       nome_cientifico AS "Nome_Cientifico", contagem
                FROM public.biota_analise_consolidada
                WHERE id_projeto = :project_id AND grupo_biologico = :group_name
                """
            ),
            connection,
            params={"project_id": PROJECT_ID, "group_name": GROUP},
    ...
    result_keys = ["Campanha", "Ponto", "Metodo_de_Captura", "Tipo_de_Amostragem", "Nome_Cientifico"]
    result_diff = compare_values(
        source_results, db_results, result_keys, "Numero_de_Individuos", "abundancia", "fonte_vs_base"
    )
    consolidated_diff = compare_values(
        source_results, consolidated, result_keys, "Numero_de_Individuos", "contagem", "fonte_vs_consolidado"
    )
    ...
    effort_diff = source_efforts[effort_keys + ["Esforco", "Unidade_Esforco"]].merge(
        db_efforts[effort_keys + ["esforco", "Unidade_Esforco"]],
        on=effort_keys,
        how="outer",
        indicator=True,
    )
    effort_diff["diferenca"] = pd.to_numeric(effort_diff["esforco"], errors="coerce") - pd.to_numeric(
        effort_diff["Esforco"], errors="coerce"
    )
    ...
    point_compare["lat_diff"] = pd.to_numeric(point_compare["latitude"], errors="coerce") - pd.to_numeric(
        point_compare["Latitude"], errors="coerce"
    )
    point_compare["lon_diff"] = pd.to_numeric(point_compare["longitude"], errors="coerce") - pd.to_numeric(
        point_compare["Longitude"], errors="coerce"
    )
    ...
    summary = {
        ...
        "fonte_taxons": source_results["Nome_Cientifico"].nunique(),
    ...
    base = args.output_dir / f"{stamp}_auditoria_pos_migracao_zoobentos_braced001"
    ...
    print(
        json.dumps(
            {"workbook": str(base.with_suffix('.xlsx')), "json": str(base.with_suffix('.json')), **summary},
            ensure_ascii=False,
            indent=2,
        )
    )
