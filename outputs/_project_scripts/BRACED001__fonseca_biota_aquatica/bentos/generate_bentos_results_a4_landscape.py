...
OUTPUT_DIR = Path(
    ...
    "Cedro Mineração/Produtos/Resultados/Bentos"
)
AUDIT_DIR = ROOT / "outputs" / "_project_scripts" / "BRACED001__fonseca_biota_aquatica" / "bentos"
...
def plot_synthesis(sheets: dict[str, pd.DataFrame], unit: str, out_name: str) -> Path:
    ...
    fig.suptitle("Síntese dos zoobentos - BRACED001", fontweight="bold", fontsize=14)
...
def write_html_report(metrics: dict, unit: str) -> Path:
    out = OUTPUT_DIR / "relatorio_tecnico_zoobentos_braced001.html"
    figures = [
...
<head>
  <meta charset="utf-8">
  <title>Resultados de Zoobentos - BRACED001</title>
  <style>
...
<body>
  <h1>Resultados de Zoobentos - BRACED001</h1>
...
def build_manifest_and_validation(generated_files: list[Path], metrics: dict, tag: str) -> tuple[Path, Path, Path, Path]:
    manifest_json = OUTPUT_DIR / "manifesto_entrega_zoobentos_braced001.json"
    manifest_xlsx = OUTPUT_DIR / "manifesto_entrega_zoobentos_braced001.xlsx"
    manifest_md = OUTPUT_DIR / "manifesto_entrega_zoobentos_braced001.md"
    validation_json = OUTPUT_DIR / "validacao_entrega_zoobentos_braced001.json"
    files = []
    for path in sorted([p for p in OUTPUT_DIR.iterdir() if p.is_file() and p.name != "desktop.ini"], key=lambda p: p.name):
        if path.name.startswith("manifesto_entrega_zoobentos_braced001"):
            continue
...
def generate(env_file: str | None = ".env") -> dict:
    ...
    export_darwincore_ief(
        ...
        output_filename="DarwinCore_IEF_Zoobentos_BRACED001.xlsx",
