...
OUTPUT_DIR = Path(
    ...
    "A&G Mineração/resultados/migracao_biota/zooplancton"
)
AUDIT_DIR = ROOT / "outputs" / "_project_scripts" / "BRAAEG001__a_g_mineracao_biota_aquatica" / "zooplancton"
...
def plot_synthesis(sheets: dict[str, pd.DataFrame], unit: str, out_name: str) -> Path:
    ...
    fig.suptitle("Síntese do zooplâncton - BRAAEG001", fontweight="bold", fontsize=15)
    return save_fig(fig, out_name)
...
def write_html_report(metrics: dict, unit: str) -> Path:
    out = OUTPUT_DIR / "relatorio_tecnico_zooplancton_braaeg001.html"
    figures = [
...
<head>
  <meta charset="utf-8">
  <title>BRAAEG001 - Relatório técnico de zooplâncton</title>
  <style>
...
<body>
  <h1>BRAAEG001 - Diagnóstico do zooplâncton</h1>
  ...
  <p>Manifesto: <code>manifesto_entrega_zooplancton_braaeg001.json</code> e <code>manifesto_entrega_zooplancton_braaeg001.xlsx</code>.</p>
</body>
</html>
"""
...
def build_manifest_and_validation(generated_files: list[Path], metrics: dict, tag: str) -> tuple[Path, Path, Path, Path]:
    manifest_json = OUTPUT_DIR / "manifesto_entrega_zooplancton_braaeg001.json"
    manifest_xlsx = OUTPUT_DIR / "manifesto_entrega_zooplancton_braaeg001.xlsx"
    manifest_md = OUTPUT_DIR / "manifesto_entrega_zooplancton_braaeg001.md"
    validation_json = OUTPUT_DIR / "validacao_entrega_zooplancton_braaeg001.json"

    files = [p for p in sorted(OUTPUT_DIR.iterdir(), key=lambda x: x.name.lower()) if p.is_file() and p.name != "desktop.ini" and not p.name.startswith("manifesto_entrega_zooplancton_braaeg001")]
    rows = []
    for path in files:
        rows.append(
            {
    ...
    lines = [
        "# BRAAEG001 - Manifesto de entrega - Zooplâncton",
        "",
...
def generate(env_file: str | None = ".env") -> dict:
    ...
    export_darwincore_ief(
        ...
        output_filename="DarwinCore_IEF_Zooplancton_BRAAEG001.xlsx",
