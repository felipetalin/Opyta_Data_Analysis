...
OUTPUT_DIR = Path(
    ...
    "A&G Mineração/resultados/migracao_biota/fitoplancton"
)
AUDIT_DIR = ROOT / "outputs" / "_project_scripts" / "BRAAEG001__a_g_mineracao_biota_aquatica" / "fitoplancton"
...
def plot_taxon_horizontal(df_taxon: pd.DataFrame, *, out_name: str, unit: str) -> Path:
    taxa_order = df_taxon.groupby("nome_cientifico")["densidade"].sum().sort_values(ascending=True).index.tolist()
    campaigns = sorted(df_taxon["nome_campanha"].unique().tolist(), key=campaign_key)
    pivot = (
        df_taxon.pivot_table(index="nome_cientifico", columns="nome_campanha", values="densidade", aggfunc="sum", fill_value=0)
    ...
    for i, (ax, campaign) in enumerate(zip(axes, campaigns)):
        ...
        if ax is not axes[0]:
            ax.tick_params(labelleft=False)
    fig.subplots_adjust(left=0.31, right=0.985, bottom=0.14, top=0.84, wspace=0.12)
    return save_fig(fig, out_name)
...
def plot_taxon_heatmap(df: pd.DataFrame, *, campaigns: list[str], points: list[str], out_name: str, unit: str) -> tuple[Path, pd.DataFrame]:
    ...
    for ax, campaign in zip(axes, campaigns):
        ...
        ax.set_yticklabels(mat.index.tolist(), fontstyle="italic", fontsize=14)
    ...
    fig.subplots_adjust(left=0.25, right=0.88, bottom=0.18, top=0.84, wspace=0.10)
...
def plot_synthesis(sheets: dict[str, pd.DataFrame], unit: str, out_name: str) -> Path:
    ...
    fig.suptitle("Síntese do fitoplâncton - BRAAEG001", fontweight="bold", fontsize=15)
    return save_fig(fig, out_name)
...
def write_html_report(metrics: dict, unit: str) -> Path:
    out = OUTPUT_DIR / "relatorio_tecnico_fitoplancton_braaeg001.html"
    figures = [
...
<head>
  <meta charset="utf-8">
  <title>BRAAEG001 - Relatório técnico de fitoplâncton</title>
  <style>
...
<body>
  <h1>BRAAEG001 - Diagnóstico do fitoplâncton</h1>
  ...
  <p>Manifesto: <code>manifesto_entrega_fitoplancton_braaeg001.json</code> e <code>manifesto_entrega_fitoplancton_braaeg001.xlsx</code>.</p>
</body>
</html>
"""
...
def build_manifest_and_validation(generated_files: list[Path], metrics: dict, tag: str) -> tuple[Path, Path, Path, Path]:
    manifest_json = OUTPUT_DIR / "manifesto_entrega_fitoplancton_braaeg001.json"
    manifest_xlsx = OUTPUT_DIR / "manifesto_entrega_fitoplancton_braaeg001.xlsx"
    manifest_md = OUTPUT_DIR / "manifesto_entrega_fitoplancton_braaeg001.md"
    validation_json = OUTPUT_DIR / "validacao_entrega_fitoplancton_braaeg001.json"

    files = [p for p in sorted(OUTPUT_DIR.iterdir(), key=lambda x: x.name.lower()) if p.is_file() and p.name != "desktop.ini" and not p.name.startswith("manifesto_entrega_fitoplancton_braaeg001")]
    rows = []
    for path in files:
        rows.append(
            {
    ...
    lines = [
        "# BRAAEG001 - Manifesto de entrega - Fitoplâncton",
        "",
...
def generate(env_file: str | None = ".env") -> dict:
    ...
    export_darwincore_ief(
        ...
        output_filename="DarwinCore_IEF_Fitoplancton_BRAAEG001.xlsx",
