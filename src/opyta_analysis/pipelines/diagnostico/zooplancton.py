...
def _load_zooplancton_df(project_id: int, group: str, env_file: str | None) -> pd.DataFrame:
...
def _run_block_4_5(df_projeto: pd.DataFrame, group: str, output_dir: Path, generated_files: list[str]) -> dict:
    if df_projeto.empty:
        tabela_export = pd.DataFrame(columns=["Taxon"])
        group_slug = _safe_group_name(group)
        xlsx = output_dir / f"04_5_tabela_ocorrencia_{group_slug}.xlsx"
    ...
    if faltando:
        raise RuntimeError(f"[ERRO] Colunas obrigatorias ausentes no Bloco 4.5 ZOO: {', '.join(faltando)}")
...
def _run_block_3(df_projeto: pd.DataFrame, group: str, output_dir: Path, generated_files: list[str]) -> dict:
    ...
    if "nome_campanha" not in df_projeto.columns or "nome_cientifico" not in df_projeto.columns:
        raise RuntimeError("[ERRO] Colunas obrigatorias ausentes no Bloco 3 ZOO: nome_campanha, nome_cientifico")
...
def _run_block_5(df_projeto: pd.DataFrame, group: str, theme: dict, output_dir: Path, generated_files: list[str]) -> dict:
    ...
    if missing:
        raise RuntimeError(f"[ERRO] Colunas obrigatorias ausentes no Bloco 5 ZOO: {', '.join(missing)}")
...
def _calc_density_zooplancton(df_: pd.DataFrame) -> tuple[pd.DataFrame, str]:
...
def _run_block_6(df_projeto: pd.DataFrame, group: str, theme: dict, output_dir: Path, generated_files: list[str]) -> dict:
    ...
    missing = [c for c in required if c not in df_projeto.columns]
    if missing:
        raise RuntimeError(f"[ERRO] Colunas obrigatorias ausentes no Bloco 6 ZOO: {', '.join(missing)}")

    col_tipo = _get_col(df_projeto, "Tipo_de_Amostragem", "tipo_de_amostragem", "tipo_amostragem")
    if col_tipo is None:
        raise RuntimeError("[ERRO] Coluna de tipo de amostragem ausente no Bloco 6 ZOO")

    col_filo = _get_col(df_projeto, "filo", "phylum")
    if col_filo is None:
        raise RuntimeError("[ERRO] Coluna de filo ausente no Bloco 6 ZOO")
    ...
    df_den, unidade = _calc_density_zooplancton(df_quant)
    if df_den.empty:
...
def _run_block_10(df_projeto: pd.DataFrame, group: str, theme: dict, output_dir: Path, generated_files: list[str]) -> dict:
    ...
    if col_tipo is None or any(c not in df_projeto.columns for c in required):
        raise RuntimeError("[ERRO] Colunas obrigatorias ausentes no Bloco 10 ZOO")
...
def _run_block_11(df_projeto: pd.DataFrame, group: str, theme: dict, output_dir: Path, generated_files: list[str]) -> dict:
    ...
    required = ["nome_campanha", "nome_ponto", "nome_cientifico", "contagem"]
    if col_tipo is None or any(c not in df_projeto.columns for c in required):
        raise RuntimeError("[ERRO] Colunas obrigatorias ausentes no Bloco 11 ZOO")
...
def _run_block_12(df_projeto: pd.DataFrame, group: str, theme: dict, output_dir: Path, generated_files: list[str]) -> dict:
    ...
    col_tipo = _get_col(df_projeto, "Tipo_de_Amostragem", "tipo_de_amostragem", "tipo_amostragem")
    if col_tipo is None:
        raise RuntimeError("[ERRO] Coluna de tipo de amostragem ausente no Bloco 12 ZOO")

    required = ["nome_ponto", "nome_cientifico", "contagem"]
    if any(c not in df_projeto.columns for c in required):
        raise RuntimeError("[ERRO] Colunas obrigatorias ausentes no Bloco 12 ZOO")
...
def run_zooplancton_pipeline(
    *,
    ...
    if block_sel in {"4", "4.5", "all"}:
        details["block_4_5"] = _run_block_4_5(
            df_projeto=df,
            group=group,
            output_dir=output_dir,
            generated_files=generated_files,
        )
        executed_blocks.append("4.5")
    ...
    if not executed_blocks:
        raise ValueError("Unsupported block for zooplancton pipeline. Use '3', '4', '4.5', '5', '6', '7', '10', '11', '12', '13' or 'all'.")
